(() => {
  ...
  const render = () => document.querySelectorAll('.methodology-course__lesson').forEach(lesson => {
    ...
    lesson.querySelector('audio').addEventListener('ended', () => button.disabled = false)
    button.addEventListener('click', async () => {
      const questions = await fetch(`http://127.0.0.1:8000/api/questions?limit=5&topic=${encodeURIComponent(topic)}`).then(r => r.json())
